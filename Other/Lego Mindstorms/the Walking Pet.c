...
//Global variables
  ...
  string death_reason;
  //Head positions
  const int HEAD_DEAD=200; //HEAD_NORMAL=60
  //Reverse enabler
  bool reverse=true;
...
task main()
{
  //Housekeeping
	  ...
	  checkBTLinkConnected();
		...
		nMotorEncoder[headM] = 0;  //clear the encoders
	...
	while (true)
	{
		...
		//To prevent the robot from reversing indefinitely, timer T3 is used for reverse timeout
		if (time1[T3] > 3200){
			reverse = false;
...
task lifeSupport()
{
  while(true)
  {
	...
	if(happy>2000)  die("Excessive Happiness");
    ...
    ClearTimer(T2);
    if(reverse) ClearTimer(T3); //Clears the timeout timer only if reverse is permitted
	 while(SensorValue[sonar] < 30 && reverse && !death) //Note: timeout is governed in main task
    {
     movelegs(leftMREV, rightMREV, timeREV);
     nxtDisplayString(3, "Reversing "+time1[T3]);
     if(time1[T3] > 800){ //Prevents messages being sent if the reverse is only temporary
       switch(random(5))
		    {
        ...
        wait1Msec(100);
        ClearTimer(T2);
     } else if(time1[T3] > 1000 && random(7)==0){
		feedback('V','1','1');
		wait1Msec(100);
		feedback('S','1','1');
		wait1Msec(100);
  ...
  while(true)
  {
    ...
    //Check for feeding - add hunger index or complain about too full
	    ...
	    if(talkAboutFood == true){
	      ...
	      if(random(15)==0) { feedback('T','0','6'); wait1Msec(50); }
	    }
  }
  return;
}
