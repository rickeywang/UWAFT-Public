...
task TOne()
{
  while(true)
  {
    ...
    nxtDisplayCenteredBigTextLine(3, "TASK 1");     // Display that TOne is running simultaneously with Main.

    nxtDisplayClearTextLine(6);                     /* Erase TTwo's display since it isn't doing anything.              */
    nxtDisplayClearTextLine(7);                     /* (Even though technically it is still running in the background.) */
  }
  return;
}
...
task TTwo()
{
  while(true)
  {
    ...
    while(SensorValue(touchSensor) == 1)            // While Touch Sensor is activated:
    {
      ...
      nxtDisplayCenteredBigTextLine(6, "TASK 2");         // Display that TTwo is running simultaneously with Main.

	    nxtDisplayClearTextLine(3);                         /* Erase TOne's display since it was killed.                          */
	    nxtDisplayClearTextLine(4);                         /* (TOne is actually gone for now so only TTwo and Main are running.) */
