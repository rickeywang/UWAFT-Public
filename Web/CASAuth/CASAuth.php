...
$wgExtensionCredits["other"][] = array(
        "name"        => "CASAuth with UWAFT Custom MySQL Auth List",
        "version"     => "2.0.3-ucb",
        "author"      => "Sullen Choirboy, Ioannis Yessios, Hauke Pribnow, Aaron Russo",
...
function casLogin($user, &$result) {
        ...
        if (isset($_REQUEST["title"])) {
                ...
                if ($_REQUEST["title"] == $lg->specialPage("CASUserlogin")) {  
						...
						*/
						...
						# Load UWAFT Authorization Module
						require("$IP/extensions/CASAuth/CASMySQLAccessList.php");
                        ...
                        if($CASAuth["RestrictUsers"]==true
                           && !uwaftAuthChk($username))
                          {
                            // redirect user to the RestrictRedirect page
                            //$wgOut->redirect($CASAuth["RestrictRedirect"]);
							echo "<p><b>CAS Login Failed. $username is not in authorized users list. Please self-register or come by to the garage to have your UW ID added.</b></p>";
                            return true;
                          }
						...
						//Send to Main Page
						echo "<meta http-equiv=\"refresh\" content=\"0;URL='/wiki/index.php?title=Main_Page'\"/>";
						echo "<p>You will be redirected in 1 second</p>";
						$wgOut->redirect('/wiki/index.php?title=Main_Page');
				 ...
				 else if ($_REQUEST["title"] == $lg->specialPage("Userlogin")) {
					echo '<p>Logging in as a traditional wiki user. </p><p> <b><a href="/wiki/index.php?title=Special:CASUserlogin">Click here to log in using UW CAS</a></b></p>';
				 }
        }
