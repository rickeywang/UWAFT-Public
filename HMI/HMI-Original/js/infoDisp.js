...
function doMasterDetailClick(panelID) {
	window.clearInterval(detailRefresh);

	var list = document.getElementById('imageList'),
		selected = list.selected;
		
	document.getElementById('title').innerHTML = selected.getTitle();
	document.getElementById('rightPanelDisp').innerHTML = document.getElementById(panelID).innerHTML ;
	document.getElementById('details').scrollTo(0,0,100);
	
	detailRefresh = setInterval(function() {
		document.getElementById('rightPanelDisp').innerHTML = document.getElementById(panelID).innerHTML ;
		//console.log('right panel refreshed: '+panelID);
    }, 40);
}
...
function masterDetail_onUnload() {
	...
	window.clearInterval(detailRefresh);
	console.log('infodisp interval cleared');
}
